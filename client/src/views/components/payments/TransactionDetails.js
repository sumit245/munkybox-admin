...
export default function TransactionDetails() {
    ...
    const pastPayments = async (id) => {
        const response = await axios.get("/api/admintochefpayments/getpastpayout/" + id)
        const { data } = response
        setPayouts(data)
    }
    useEffect(() => {
            ...
            <div className='ibox my-2'>
                ...
                <div className='ibox-content'>
                    <div className='form-inline' >
                        ...
                        <span className='mx-1'>{moment(payout_start_date).format("Do MMM")}</span> -
                        <span className='mx-1'> {moment(payout_end_date).format("Do MMM")}</span>
                    </div>
                    <div className='row'>
                        <div className='col-sm-4 border-right'>
                            <h5>Net Earning</h5>
                            <span>${parseFloat(parseFloat(totalBaseIncome) + parseFloat(totalAddOnRevenue)) - (parseFloat(parseFloat(totalBaseIncome) + parseFloat(totalAddOnRevenue)).toFixed(2) * 0.1)}</span>
                        </div>
                        <div className='col-sm-4 border-right'>
                            <div className='form-inline'>
                        ...
                        <div className='col-sm-4'>
                            <h5>Next Paymet</h5>
                            <span>{moment(payout_end_date).add(2, "days").format("Do MMM")}</span>
                        </div>
                    </div>
                </div>
            </div>
